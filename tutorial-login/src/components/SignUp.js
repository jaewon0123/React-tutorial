...
const SignUp = () => {
  ...
  const 아이디중복검사 = (eventId) => {
    ...
    .then(result => {
        ...
        // 결과전달을 0을 사용하지 않고 "중복아님"글자 전달 if(result)
        if (Number(result) === 0) {
  ...
  const 회원가입버튼 = () => {
    ...
    */
    ...
    fetch("/signUp", {
        ...
        // applicaion/json : applicaion=코딩하는폴더 자체를 의미 폴더1개 = 어플리케이션 1개
        // json = 사용자나 개발자가 작성한 여럭밧을 key-value형태로 데이터를 1개이상 주고 받을 수 있는 형태
  ...
  return (
    <div className="signup-container">
      {/* 
        ...
        <label for="input에 작성된 id값과 일치하는 앖을 작성"></label>
        <label><inpur/></label> 작성해주면 for를 작성한 것과 동일하게 설정
